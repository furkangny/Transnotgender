/*
 * RabbitMQ Client
 * Message queue client for relationships service
 */
...
class RabbitMQClient {
  ...
  async connect() {
    ...
    try {
        ...
        this.activeConnection.on('error', (err) => console.log('RabbitMQ connection error: ', err));
        this.activeConnection.on('close', () => this.#attemptReconnect());

        this.msgChannel = await this.activeConnection.createChannel();
        this.msgChannel.on('error', async (err) => { console.log('RabbitMQ channel error: ', err); });
        ...
        await this.msgChannel.bindQueue(this.queueName, this.exchangeName, 'relationships.#');
