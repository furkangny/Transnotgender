...
class GameEventBus {
    ...
    async connect() {
        ...
        try {
            ...
            this.connection.on('error', (error) => console.log('GameEventBus connection error: ', error))
            this.connection.on('close', () => this.#attemptReconnect())

            this.channel = await this.connection.createChannel();
            this.channel.on('error', async (error) => { console.log('GameEventBus channel error: ', error); });
            await this.channel.assertExchange(this.exchange, this.exchangeType, { durable: true });
            await this.channel.assertQueue(this.queueName, { durable: true });
            await this.channel.bindQueue(this.queueName, this.exchange, 'game.#');

            this.isConnecting = false;
            this.reconnectAttempts = 0;
        } catch (error) {
            console.log('Failed to connect to GameEventBus: ', error);
            this.isConnecting = false;
