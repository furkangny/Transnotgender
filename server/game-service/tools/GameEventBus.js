/*
 * Game Event Bus
 * RabbitMQ client for game service
 */
...
class GameEventBus {
    ...
    async connect() {
        ...
        try {
            ...
            this.activeConnection.on('error', (err) => console.log('GameEventBus connection error: ', err));
            this.activeConnection.on('close', () => this.#attemptReconnect());

            this.msgChannel = await this.activeConnection.createChannel();
            this.msgChannel.on('error', async (err) => { console.log('GameEventBus channel error: ', err); });
            ...
            await this.msgChannel.bindQueue(this.queueName, this.exchangeName, 'game.#');

            this.isConnecting = false;
            this.reconnectCount = 0;
        } catch (err) {
            console.log('Failed to connect to GameEventBus: ', err);
            this.isConnecting = false;
